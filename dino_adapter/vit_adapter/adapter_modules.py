...
def deform_inputs(x, up=False):
    ...
    if up:
        reference_points = get_reference_points([((h * 2) // 14, (w * 2) // 14)], x.device)
    else:
    ...
    if up:
        spatial_shapes = torch.as_tensor([((h * 2)  // 14, (w * 2) // 14)], dtype=torch.long, device=x.device)
    else:
