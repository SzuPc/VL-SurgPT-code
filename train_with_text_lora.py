...
from lora_model.lora import inject_lora_into_offset_head
...
def train(args, train_dataloader, model, optimizer, lr_scheduler, scaler, epoch):
    ...
    for i, batch in enumerate(train_dataloader):
        ...
        video = video[:, :30]
        gt_points = gt_points[:, :, :30]
        gt_visibility = gt_visibility[:, :, :30]
...
def main_worker(args):
    ...
    # inject_lora_into_offset_head(model, r=8, alpha=16.0)
