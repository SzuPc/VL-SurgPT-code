...
STATUS_CLASS_NAMES = [
    "Eternal occlusion", "Self occlusion", "Off camera", "Clear view"
]
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc="Evaluating"):
            ...
            for t in tqdm(range(1, T), desc="Processing frames", leave=False):
                ...
                coords = output["correct_pred"][0].cpu().numpy()  # (N, 2)
            ...
            with open(save_path / "point_and_status.json", "w") as f:
                json.dump(all_frame_outputs, f, indent=2)

            logger.info(f"Saved results to {save_path / 'point_and_status.json'}")
