...
STATUS_CLASS_NAMES = [
    "Clear view", "Self occlusion", "Eternal occlusion", "Off camera"
]
...
def smart_init_queries(trackon, gt_query, prev_pred_coords, frame, scale):
    """
    gt_query: Tensor (N, 2), 原图尺度
    prev_pred_coords: Tensor (N, 2), 原图尺度
    scale: [scale_x, scale_y]
    """
    device = gt_query.device
    scale_x, scale_y = torch.tensor(scale, device=device)

    query = torch.zeros_like(gt_query)

    for i in range(gt_query.shape[0]):
        if (gt_query[i] != -1).all():
            query[i, 0] = gt_query[i, 0] 
            query[i, 1] = gt_query[i, 1]
        else:
            query[i, 0] = prev_pred_coords[i, 0]
            
            query[i, 1] = prev_pred_coords[i, 1]

    trackon.init_queries_and_memory(query, frame)
...
def smooth_trajectory_with_gt(pred_coords: np.ndarray, gt_coord: np.ndarray):
    kf = init_kf()
    smoothed = []

    x0, y0 = pred_coords[0]
    vx = pred_coords[1, 0] - x0 if len(pred_coords) > 1 else 0
    vy = pred_coords[1, 1] - y0 if len(pred_coords) > 1 else 0
    kf.x = np.array([x0, vx, y0, vy])

    for t in range(len(pred_coords)):
        kf.predict()
        if t == len(pred_coords) - 1:
            kf.update(gt_coord)
        else:
            kf.update(pred_coords[t])
        smoothed.append(kf.x[[0, 2]])  # x, y
...
def inference():
    ...
    # ==== 配置 ====
    dataset_root = Path("/home/lenovo/acm_mm_dataset/instrument_text_guidance_dataset/test")
    ...
    tg_ckpt_path = "/home/lenovo/acm_mm_dataset/track_on_text/checkpoint_instrument/text_guidance_shortclip_checkpoint_epoch_10.pt"
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc="Evaluating"):
            ...
            segment_frame_indices = []
                ...
                # 初始化上一帧预测 coords（默认用第0帧 GT）
            prev_pred_coords = gt_points[:, 0, :].clone()

            for t in tqdm(range(T), desc="Processing frames", leave=False):
                frame = video[t].unsqueeze(0)
                if t % 30 == 0:
                    gt_query = gt_points[:, t, :]  # shape: (N, 2)
                    smart_init_queries(trackon, gt_query, prev_pred_coords, frame, scale)

                frame_out = trackon.ff_forward(frame)
                coord_pred = frame_out["coord_pred"]  # (N, 2)
                q_t = frame_out["q_t"]
                ...
                coords[:, 0] /= scale[0]
                coords[:, 1] /= scale[1]
                prev_pred_coords = torch.tensor(coords, device=gt_points.device)
                ...
                segment_pred_coords.append(coords)
                segment_frame_indices.append(t)

                    # === 每段结束后进行滤波并写入输出 ===
                if (t + 1) % 30 == 0 or t == T - 1:
                    if (gt_points[:, t, :] != -1).all(dim=1).any():
                        gt_coord = gt_points[:, t, :].cpu().numpy()
                    else:
                        gt_coord = segment_pred_coords[-1]  # 无 GT 时 fallback 到最后预测

                    smoothed_segment = []
                    for i in range(len(coords)):  # 每个点单独滤波
                        pred_seq = np.array([segment_pred_coords[k][i] for k in range(len(segment_pred_coords))])
                        smoothed = smooth_trajectory_with_gt(pred_seq, gt_coord[i])
                        smoothed_segment.append(smoothed)

                    smoothed_segment = np.stack(smoothed_segment, axis=1)  # [L, N, 2]

                    for coords, frame_idx in zip(smoothed_segment, segment_frame_indices):
                            all_frame_outputs.append({
                                "frame_index": frame_idx,
                                "coordinates": coords.tolist(),
                                "status_preds": [STATUS_CLASS_NAMES[i] for i in status_ids],
            ...
            save_path = Path("/home/lenovo/acm_mm_dataset/vis_track_status/instrument") / case / side / seq
            save_path.mkdir(parents=True, exist_ok=True)

            with open(save_path / "kalman_point_and_status.json", "w") as f:
                json.dump(all_frame_outputs, f, indent=2)

            logger.info(f"Saved results to {save_path / 'kalman_point_and_status.json'}")
